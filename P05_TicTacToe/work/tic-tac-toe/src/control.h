 ...
 */
typedef struct {
    size_t line[3]; ///< the sequence of cells (1...9) or 0 in the first element
                    ///< if no win
} control_line_t;
 ...
 * @remark                   Silently ignores a move if it is not allowed (e.g.
 * if already completed or the field is already played, etc.).
 */
 ...
 * @returns                  Returns the player which played this field (if
 * any).
 */
 ...
 * @param  instance [INOUT]  The instance which holds the state.
 * @returns                  Returns the field numbers in increasing order
 * (1...9) which win the game (if any).
 * @remark                   If there is no winner (yet), the first entry in the
 * result is 0.
 */
