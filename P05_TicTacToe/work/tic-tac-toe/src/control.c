...
/**
 * @brief            Conversion from control field number (1...9) to 0-based
...
/**
 * @brief           Conversion from 0-based model position (row, col) to control
 * field number (1...9).
 ...
 * @return                  Returns 0 if no move is possible any more,
 * otherwise 1.
 */
...
void control_move(control_t* instance, size_t cell) {
    assert(instance);
    if (model_move(instance->model, get_pos(cell),
                   get_state(instance->player))) {
...
control_line_t control_get_win(control_t* instance) {
    ...
    switch (line.dir) {
        case model_dir_h:
            return (control_line_t) {
                {start_cell, start_cell + 1, start_cell + 2}};
        case model_dir_v:
            return (control_line_t) {
                {start_cell, start_cell + 3, start_cell + 6}};
        case model_dir_d:
            if (start_cell == 1) {
                return (control_line_t) {
                    {start_cell, start_cell + 4, start_cell + 8}};
            } else {
                return (control_line_t) {
                    {start_cell, start_cell + 2, start_cell + 6}};
            }
        default:
