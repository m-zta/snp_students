...
/**
 * @brief  Winner line direction - the winner line is given together with the
 * start position.
 */
typedef enum {
 ...
 * @param  instance [INOUT]  The instance which holds the state.
 * @return                   Returns the wining player or model_state_none if no
 * winner (yet).
 */
model_state_t model_get_winner(model_t* instance);

/**
 * @brief                    Queries if a move is possible (i.e. not won yet and
 * any field available?).
 * @param  instance [INOUT]  The instance which holds the state.
 ...
 * @param  state [IN]        The new state (only model_state_a and model_state_b
 * allowed).
 * @return                   Returns if the attempt to move was successful.
 * @remark                   Does only succeed if not yet won and if the field
 * is available.
 */
 ...
 * @remark                   The start position is 0/0, 1/0, 2/0 for horizontal,
 * 0/0, 0/1, 0/2 for vertical, 0/0, 0/2 for diagonal.
 */
