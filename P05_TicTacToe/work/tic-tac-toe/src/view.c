...
#define AVAILABLE                                                              \
    "\033[40m" ///< ANSI termial CSI sequence for available fields (black)
#define PLAYER_A                                                               \
    "\033[42m" ///< ANSI termial CSI sequence for one player (green)
#define PLAYER_B                                                               \
    "\033[41m"         ///< ANSI termial CSI sequence for the other player (red)
...
#define SIZE                                                                   \
    3 ///< size of the game to avoid magic numbers in the code (not meant to
      ///< modify)
#define CELLS                                                                  \
    (SIZE * SIZE) ///< size of the game to avoid magic numbers in the code (not
                  ///< meant to modify)
...
/**
 * @brief              Displays a sequence of spaces at the given position in
 * the given background color.
 * @param  row [IN]    position parameter
 * @param  col [IN]    position parameter
 * @param  width [IN]  how many spaces to write
 * @param  color [IN]  the format string before writing the spaces (intent:
 * background color)
 * @remark             After writing the spaces, the format is reset.
 */
static size_t show_bar(size_t row, size_t col, size_t width,
                       const char* color) {
    goto_pos(row, col);
    printf("%s", color);
...
/**
 * @brief              Writes for the call at position y/x the given number with
 * the given background color.
 ...
 * @param  color [IN]  the format string before writing the text (intent:
 * background color)
...
/**
 * @brief              Renders the given cell with the given background color,
 * including the surrounding border.
 * @param  n [IN]      the cell number (0...CELLS-1)
 * @param  color [IN]  the format string for the cell content (intent:
 * background color)
 * @remark             After writing the number, the format is reset.
 */
static void show_cell(size_t n, const char* color) {
    // goto upper-left corner of a cell (the cell starts with an upper and left
    // gap)
...
/**
 * @brief               Renders the given player's name in the given background
 * color.
 * @param  player [IN]  the player to render (select the background color and
 ...
 */
static void show_player(size_t row, size_t col, const char* label,
                        control_player_t player) {
    goto_pos(row, col);
    printf(RESET);
    col += printf("%s", label);
    goto_pos(row, col);
...
/**
 * @brief                 Renders the board from the status given by the control
 * instance.
...
static void show(view_t* instance) {
    ...
    puts(CLS);
    show_status(control_get_winner(instance->control),
...
void view_run(view_t* instance) {
    if (isatty(STDIN_FILENO)) { // in case of an interactive terminal, the
                                // exhoing and buffering is disabled
        // declare non POSIX function, which is available in glibc, but not in
        // strinct C99 mode
