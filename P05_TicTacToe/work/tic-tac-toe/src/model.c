...
model_state_t model_get_state(model_t* instance, model_pos_t pos) {
    ...
    // Instructions to the students:
    // replace the stub implementation my access to the field at the given
    // position. BEGIN-STUDENTS-TO-ADD-CODE
...
model_state_t model_get_winner(model_t* instance) {
    ...
    return line.dir == model_dir_none
               ? model_state_none
               : model_get_state(instance,
                                 model_pos(line.start.row, line.start.col));
}
...
int model_can_move(model_t* instance) {
    ...
    if (model_get_winner(instance) == model_state_none) {
        // Instructions to the students:
        // scan all fields: return 1 with first field which equals
        // model_state_none BEGIN-STUDENTS-TO-ADD-CODE
...
int model_move(model_t* instance, model_pos_t pos, model_state_t state) {
    assert(instance);
    assert_pos(pos);
    if (model_get_state(instance, pos) == model_state_none &&
        model_can_move(instance)) {
